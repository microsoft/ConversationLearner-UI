...
export default {
    'en-US': {
        ...
        [FM.PROFILE_SETTINGS_BOT_PORT_DESCRIPTION]: 'Normally this administrative interface is served from the bot so it implicitly knows PORT ({ port }) which it is loaded from is also the bot port and will use this by default. For special cases such as development you can use the override below to use a custom port.',
