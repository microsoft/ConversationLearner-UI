/**
 * These are all of the types that can be created by the loader.
 *
 ...
 *
 * Step which begins another dialog (and when that dialog is done, it will return the
 * caller).
 *
 * Command to cancel all of the current dialogs by emitting an event which must be caught to
 * prevent cancelation from propagating.
 *
 ...
 *
 * This represents a dialog which gathers Date or Time or DateTime from the user
 *
 * This is a step which allows you to remove a property from memory
 *
 * This is a step which allows you to modify an array in memory
 *
 * Edit current dialog with changeType and Steps
 *
 * This is a step which allows you to emit an event
 *
 * Command which ends the current dialog, returning the resultProperty as the result of the
 * dialog.
 *
 * End the current turn without ending the dialog.
 *
 * Defines a rule for an event which is triggered by some source
 *
 * Step which executes steps per item in a collection.
 *
 * Step which execute steps per item page in a collection.
 *
 * This is a step which replaces the current dialog with the target dialog
 *
 * Step which conditionally decides which step to execute next.
 *
 * This step allows you to innitial a property to either an object or array
 *
 ...
 *
 * This is a step which writes to console.log and optional creates a TraceActivity around a
 * text binding
 *
 * LUIS recognizer.
 *
 ...
 *
 * This represents a dialog which gathers a decimal number in a specified range
 *
 * This represents a dialog which gathers an OAuth token from user
 *
 * Recognizer which uses regex expressions to generate intents and entities.
 *
 * This is a step which repeats the current dialog with the same dialog.
 *
 * This is a step which sends an activity to the user
 *
 * This step allows you to set memory to the value of an expression
 *
 * This represents a dialog which gathers a text from the user
 *
 * This is a step which sends an TraceActivity to the transcript
 *
