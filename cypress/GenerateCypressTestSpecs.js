...
function main() {
  try {
    ...
    let testList_jsContents = fs.readFileSync(pathToTestList, 'utf8')

    // Get the list of test js files.
    let testFileDirectory = fs.readdirSync(pathToTestFiles)
    ...
    let index = testList_jsContents.indexOf(generatedCode)
    ...
    let newFileContents = generatedCode + '\r\n' +
...
function ParseTestJsFile(fileName) {
  let fileContents = fs.readFileSync(`${pathToTestFiles}/${fileName}`, 'utf8')
  ...
  fileLines.forEach(line => {
    ...
    if (line.startsWith(CypressTestCase)) {
      ...
      let iStart = line.indexOf("'", CypressTestCase.length) + 1
      let iEnd = line.indexOf("'", iStart)
      let groupName = line.substring(iStart, iEnd)
  
      iStart = line.lastIndexOf(",") + 1
      iEnd = line.lastIndexOf(')')
      let testName = line.substring(iStart, iEnd).trim()
...
function CreateSpecFile(groupName, testName) {
  let folderPath = `${pathToTestSpecFiles}/${groupName}`
  if (!fs.existsSync(folderPath)) { fs.mkdirSync(folderPath) }
  
  let filePath = `${folderPath}/${testName}.js`
  if (!fs.existsSync(filePath)) {
    let newFileContents = `require('../../support/TestListManager').AddToCypressTestList('${groupName}.${testName}')`
...
function RemoveUnusedSpecFiles() {
  let nothingWasDeleted = true
  // Get a list of directory entries for folders under the Cypress test spec file.
  let directoryEntries = fs.readdirSync(pathToTestSpecFiles, {withFileTypes: true})

  directoryEntries = directoryEntries.filter(directoryEntry => directoryEntry.isDirectory())
  
  directoryEntries.forEach(directoryEntry => {
    let workingPath = `${pathToTestSpecFiles}${directoryEntry.name}`
    let specFileList = fs.readdirSync(workingPath)
    
    specFileList.forEach(specFileName => {
      if(specFileName.endsWith('.js')) {
        let groupFileSpecification = `${directoryEntry.name}.${specFileName.substring(0, specFileName.length - 3)}`
